...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!existentProducts.length) {
      throw new AppError('could not find any products with id given');
    }
    ...
    if (findProductsWithNoQuantityAvailable.length) {
      throw new AppError(
        `the quantity ${findProductsWithNoQuantityAvailable[0].quantity} is nott available for ${findProductsWithNoQuantityAvailable[0].id}`,
      );
    }
